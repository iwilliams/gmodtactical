...
function SWEP:canAttachSpecificAttachment(attachmentName, ply, imaginaryAttachments, imaginaryActiveAttachments, lookIn)
	...
	for category, data in pairs(self.Attachments) do
		for key, attachment in ipairs(data.atts) do
			if attachment == attachmentName then
				local att = CustomizableWeaponry.registeredAttachmentsSKey[attachment]
				
				if att.dependencies then
					local can, dependency = CustomizableWeaponry.canBeAttached(self, att, imaginaryAttachments)
					
					if not can then
						return false, -1, dependency
					end
				end
				
				if data.exclusions then
					local has, exclusion = self:hasExcludedAttachment(data.exclusions, imaginaryActiveAttachments)
					
					if has then
						return true, -2, exclusion
					end
				end
				
				-- checking whether it has dependencies or not saves us a loop
				if CustomizableWeaponry:hasAttachment(ply, att.name, lookIn) then
					local can, result, returnData = self:isAttachmentEligible(att.name, imaginaryActiveAttachments)

					if not can then
						return false, (result == -1 and -3 or -4), returnData -- -3 exclusion; -4 dependency
					end

					local can, result, returnData = self:isCategoryEligible(data.dependencies, data.exclusions, imaginaryActiveAttachments)
					
					if not can then
						return false, (result == -1 and -5 or -6), returnData -- -5 exclusion; -6 dependency
					end
