...
function SWEP:isAttachmentEligible(name, activeAttachments)
	...
	if self.AttachmentExclusions then
		...
		if excl then
			for k, v in pairs(excl) do
				if activeAttachments[v] then
					return false, -1, activeAttachments[v] -- active attachment excludes
				end
			end
		end
	end
	...
	return found, -2, self.AttachmentDependencies[name] -- in case of false - attachment we depend on is not attached
end
