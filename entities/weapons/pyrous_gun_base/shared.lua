...
function SWEP:PrimaryAttack()
	...
	if !self.Owner:KeyDown(IN_SPEED) and !self.Owner:KeyDown(IN_RELOAD) then
		...
		if GetConVar("pspak_gas_effect") != nil then
			if GetConVar("pspak_gas_effect"):GetBool() then 
...
function SWEP:CheckWeaponsAndAmmo()
	if SERVER and self.Weapon != nil and (GetConVar("pspak_weapon_stripping"):GetBool()) then 
...
function SWEP:ShootBullet(damage, recoil, num_bullets, aimcone)
	...
	if SERVER and game.SinglePlayer() and !self.Owner:IsNPC()  then 
		...
		if GetConVar("pspak_dynamic_recoil"):GetBool() then
...
function SWEP:RicochetCallback(bouncenum, attacker, tr, dmginfo)
	...
	if GetConVar("pspak_disable_penetration_ricochet") == nil then
		...
		PenetrationChecker = GetConVar("pspak_disable_penetration_ricochet"):GetBool()
	end
...
function SWEP:TossOrHolster()
	...
	--if (gmod.GetGamemode().Name == "Murderthon 9000") or GetConVar("pspak_debug_weaponry"):GetBool() then
	//everything you need to make a weapon-tossing function goes here
	--end
	
	if (GetConVar("pspak_disable_holstering"):GetBool()) then return end
...
if not (GetConVar("pspak_unique_slots"):GetBool()) then 
SWEP.SlotPos = 2
end